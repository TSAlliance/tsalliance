        ...
        for(let group of this.routes) {
            for(let route of group.routes) {
                this.expressApp[route.method.toLowerCase()](route.path, async (request: Request, response: Response) => {
                    ...
                    let account: Account | TrustedError = await Account.authenticateRequest(request)
                    ...
                    if(!account || account instanceof TrustedError && requiresAuth) {
                        this.sendErrorResponse(response, account as TrustedError)
                        return
                    }

                    // Define current route
                    let currentRoute = new Router.Route(group, route.name, route.path, route.action, account, request, response)
                    currentRoute.isOwnResource = this.isOwnResourceRequest(currentRoute)
                    this.translateScopes(currentRoute)

                    if(!(account instanceof TrustedError) && !account?.hasPermission(currentRoute.permission, currentRoute) && requiresAuth) {
    ...
    private translateScopes(route: Router.Route): Router.Route {
        for(let prop in route.params) {
            if(route.params[prop] === "@me") {
                route.params[prop] = route.account?.["uuid"]
            }
        }
...
export namespace Router {
    ...
    export class Route {
        ...
        public readonly account: Account | TrustedError
        ...
        constructor(group: Routes.RouteGroup, name: String, path: String, action: String, account: Account | TrustedError, request: Request, response: Response) {
            this.group = group
            this.name = name
            this.path = path
            this.action = action
        ...
        private ownResource() {
            try {
                return this.params?.["uuid"] == this.account["uuid"]
            } catch (error) {
                return false
            }
        }
